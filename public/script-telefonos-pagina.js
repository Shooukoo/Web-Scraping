...
// Función para limpiar productos duplicados
function removeDuplicateProducts(products) {
    const seen = new Map();
    const cleanProducts = [];
    
    for (const product of products) {
        // Crear una clave única basada en modelo y descripción
        const key = `${product.model}-${product.description}`.toLowerCase().trim();
    ...
    console.log(`Productos originales: ${products.length}, Después de limpiar: ${cleanProducts.length}`);
    return cleanProducts;
}

// Quick Sort para ordenar productos
function quickSortProducts(products, compareFunction) {
    ...
    for (const product of products) {
        const comparison = compareFunction(product, pivot);
        if (comparison < 0) {
            left.push(product);
    ...
    return [
        ...quickSortProducts(left, compareFunction),
        ...equal,
        ...quickSortProducts(right, compareFunction)
    ];
}

// Función de comparación para ordenar por marca y luego por nombre
function compareByBrandAndName(a, b) {
    const brandA = a.model.split('|')[0].trim();
    const brandB = b.model.split('|')[0].trim();
...
// Función principal que limpia y ordena
function cleanAndSortProducts(products, sortBy = 'brand') {
    // Primero limpiar duplicados
    let cleanProducts = removeDuplicateProducts(products);
    
    // Luego ordenar por marca y nombre
    if (sortBy === 'brand') {
        cleanProducts = quickSortProducts(cleanProducts, compareByBrandAndName);
    }
...
async function fetchPhoneData() {
    ...
    try {
        ...
        allProducts = cleanAndSortProducts(data, 'brand');
        filteredProducts = [...allProducts];
        availableBrands = getAvailableBrands(allProducts);   
